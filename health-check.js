...
(function () {
  ...
  window.hcAutoFixAll = async function () {
    ...
    var ok = window.confirmDialog
      ...
      /* 🔴 كان الاحتياطي هنا `true` — أي: لو تعذّر تحميل نظام التأكيد
         لأي سبب، يُنفَّذ **حذف جماعي للمباريات بلا أي سؤال**. وهو أخطر
         مسار في اللوحة كلها (danger:true). المسار الفردي أعلاه يسقط
         إلى confirm() الأصلي بشكل صحيح — فنفعل مثله تماماً. */
      : confirm('⚡ إصلاح كل ما يمكن إصلاحه\n\nسيُنفَّذ ' + idxs.length +
                ' إصلاحاً تلقائياً، وبعضها يحذف مباريات مكرّرة.\n\nمتابعة؟');
    if (!ok) return;
    var total = 0;
