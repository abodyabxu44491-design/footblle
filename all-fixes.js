...
(function() {
  ...
  // ════════════════════════════════════════════════════════════════
  //  FIX-3  [مُعطَّل عمداً] — كان يعيد كتابة بطاقة المجموعة بتصميم مختلف
  // ════════════════════════════════════════════════════════════════
  // كانت هذه الدالة تلفّ window.renderGroupsAdmin وتعيد رسم #groupsAdminList
  // بتصميم "جدول نقاط" منفصل تماماً عن تصميم البطاقة الأصلي في admin.js
  // (الشعارات + بانر المباراة الفاصلة + ترتيب أزرار تأهيل/خروج المتقن).
  // بما أن admin.js يعيد رسم نفس العنصر عند أي تحديث (كل ضغطة تأهيل/خروج
  // تُطلق onSnapshot ثم renderGroupsAdmin من جديد)، كانت النتيجة أن التصميم
  // الجميل يظهر لحظة ثم "يرتد" فوراً لتصميم هذا الـFIX الأبسط — وهذا بالضبط
  // ما كان يسبب مشكلة "الشكل يرجع القديم بمجرد الضغط على تأهيل/خروج".
  // الحل: تعطيل هذا الـFIX نهائياً وترك admin.js وحده مسؤولاً عن رسم
  // #groupsAdminList — تصميمه الأصلي already يحتوي الزرين (تأهيل/خروج)
  // والشعارات وبانر المباراة الفاصلة، فيبقى ثابتاً بلا أي ارتداد.
  if (false && IS_ADMIN) {
    waitFor(
