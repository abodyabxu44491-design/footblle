...
window.openPlayerModal = function(playerName, teamId, playerId) {
  ...
  matches.filter(m => m.status === 'finished').forEach(m => {
    ...
    /* 🔴 كان يشترط المعرّف متى وُجد لدى اللاعب، **بلا رجوع إلى الاسم**:
       `player.playerId ? (pl.id && pl.id === player.playerId) : ...`
       ومدخل التشكيلة قد يُحفظ بلا معرّف (يُكتب الاسم يدوياً في المحرّر بدل
       اختياره من الكشف — وهو الشائع). فتفشل المطابقة وتضيع المشاركة، فيظهر
       اللاعب بمباريات أقلّ مما لعب فعلاً.
       القاعدة الصحيحة: **قارن بالمعرّف متى توفّر عند الطرفين معاً**، وإلا
       قارن بالاسم — وهي القاعدة المطبَّقة أصلاً في مطابقة أحداث التبديل،
       فتوحيدها يمنع اختلاف النتيجة بين الحسابين. */
    const _plMatch = (pl) => (player.playerId && pl.id)
      ? pl.id === player.playerId
